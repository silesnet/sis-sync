...
public class PohodaImportStubTest extends XMLTestCase {
    ...
    @Test
    public void testSpringFragmentReader() throws UnexpectedInputException, NoWorkFoundException, ParseException,
            Exception {
    ...
    @Test
    public void testXStreamMapping() throws UnexpectedInputException, NoWorkFoundException, ParseException, Exception {
        Resource input = new ClassPathResource("data/20081206_sis_customers.xml");
        ...
        reader.setFragmentDeserializer(new EventReaderDeserializer() {
            public Object deserializeFragment(XMLEventReader eventReader) {
                ...
                try {
                    ...
                    // StaxReader staxReader = new StaxReader(new QNameMap(), streamReader);
                    // PathTrackingReader trackingReader = new PathTrackingReader(staxReader, new PathTracker());
    ...
    public void testStaxMate() throws XMLStreamException, IOException, SAXException {
        ...
        for (items.advance(); items.asEvent() != null; items.advance()) {
            ...
            SMInputCursor addressbookHeader = items.childElementCursor().advance().childElementCursor().advance();
            // identity/address
            SMInputCursor address = addressbookHeader.childElementCursor().advance().childElementCursor().advance();
            ...
            for (addressChildren.advance(); addressChildren.asEvent() != null; addressChildren.advance()) {
                System.out.println(addressChildren.getLocalName() + ": " + addressChildren.collectDescendantText());
            }
        }
    ...
    private String cursorLocation(SMInputCursor cursor) {
        ...
        try {
            location = cursor.getCursorLocation().getLineNumber() + ", " + cursor.getCursorLocation().getColumnNumber();
        } catch (XMLStreamException e) {
            throw new RuntimeException(e);
        }
        return location;
    ...
    public class DataPackItemReader extends NodeFactory {
        ...
        public Nodes finishMakingElement(Element element) {
            ...
            Element details = new Element("rdc:producedDetails", "http://www.stormware.cz/schema/documentresponse.xsd");
